...
class VOInitializer:
    ...
    def _detect_and_match_features(self, 
        ...
        return kpset1, kpset2, good_matches
    ...
    def _check_reconstruction_quality(self, points3D: np.ndarray):
        ...
        if np.median(depths) < 0:
            ...
            return False
        if len(points3D) < 50:
            print(f"Insufficient 3D points for reliable reconstruction: {len(points3D)}")
            return False
        return True
    ...
    def initialize(self, 
        ...
        kpset1, kpset2, matches = self._detect_and_match_features(frame1, frame2)
        pts1 = np.array([kp.pt for kp in kpset1], dtype=np.float32)
        pts2 = np.array([kp.pt for kp in kpset2], dtype=np.float32)
        if len(matches) < self.min_matches:
            print(f"Too few matches: {len(matches)} < {self.min_matches}")
            return False, None, None, None, None, None
        
        # mask：ransac得到的布尔掩码，标记哪些匹配点是内点（值为 1）还是外点（值为 0）
        ...
        # 使用内点掩码过滤关键点
        inlier_kpset1 = [kpset1[i] for i in range(len(kpset1)) if mask.ravel()[i] == 1]
        inlier_kpset2 = [kpset2[i] for i in range(len(kpset2)) if mask.ravel()[i] == 1]
        
        if not self._check_reconstruction_quality(points3D):
            print("Poor 3D reconstruction quality")
            return False, None, None, None, None, None
        
        return True, R, t, points3D, inlier_pts1, inlier_pts2, inlier_kpset1, inlier_kpset2